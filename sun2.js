...
class TaiXiuAnalyzer {
    ...
    runSubModelStreak(results, model) {
        ...
        switch (model.logic) {
            ...
            case 'break':
                if (streak >= 4) {
                    return { prediction: other, confidence: 0.6 + (streak * 0.03), reason: `Bệt ${streak} phiên, dự đoán sắp gãy` };
                }
                break;
            case 'hybrid':
                if (streak >= 3) {
                    ...
                    if (prev && prev !== last) {
                        return { prediction: last, confidence: 0.7, reason: `Bệt sau khi đảo từ ${prev}` };
                    }
                }
                break;
            case 'super':
        ...
        switch (model.logic) {
            ...
            case 'variant':
                if (last12.length === 12) {
                    ...
                    if (score >= 3) {
                        return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (score * 0.05), reason: `Cầu 3-3 biến thể với ${score}/4 bộ ba` };
                    }
                }
                break;
            case 'short':
        ...
        switch (model.logic) {
            ...
            case 'long':
                if (results.length >= 10) {
                    ...
                    if (count >= 2) {
                        return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 2-1-2 dài hạn' };
                    }
                }
                break;
        }
        ...
        switch (model.logic) {
            ...
            case 'long':
                if (results.length >= 10) {
                    ...
                    if (count >= 2) {
                        return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 1-2-1 dài hạn' };
                    }
                }
                break;
        }
    ...
    runSubModelAdvanced(results, model) {
        ...
        switch (model.logic) {
            ...
            case 'symmetry':
                ...
                if (symmetry.found) {
                    return { prediction: symmetry.prediction, confidence: 0.75, reason: 'Phát hiện cầu đối xứng' };
                }
                break;
            case 'fibonacci':
                const fib = this.checkFibonacci(results);
                if (fib.found) {
                    return { prediction: fib.prediction, confidence: 0.7, reason: 'Phát hiện cầu Fibonacci' };
                }
                break;
            case 'longTrend':
        ...
        if (predictions.length > 0) {
            ...
            if (topFaces.length >= 3) {
                ...
                if (predictedScores.length > 0) {
                    ...
                    return { prediction: predType, confidence: 0.65, predicted_faces: topFaces.map(f => f.face), reason: `Dựa trên biến động xúc xắc, các mặt ${topFaces.map(f => f.face).join(',')} có khả năng xuất hiện cao` };
                }
                ...
                if (predictedScores.length > 0) {
                    const avgPredicted = predictedScores.reduce((a, b) => a + b, 0) / predictedScores.length;
                    const predType = avgPredicted >= 11 ? 'Tài' : 'Xỉu';
                    return { prediction: predType, confidence: 0.6, predicted_faces: topFaces.map(f => f.face), reason: `Dựa trên biến động xúc xắc, các mặt ${topFaces.map(f => f.face).join(',')} có khả năng xuất hiện cao` };
                }
            } else {
                ...
                const predType = avgPredicted >= 11 ? 'Tài' : 'Xỉu';
                return { prediction: predType, confidence: 0.55, predicted_faces: [face], reason: `Dựa trên biến động xúc xắc, mặt ${face} có khả năng xuất hiện cao` };
            }
        }
